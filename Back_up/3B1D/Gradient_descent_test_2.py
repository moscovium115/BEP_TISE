...
print("norm V matrix:", sp.sparse.linalg.norm(V/E_target))
Q=np.kron(B,np.eye(n))+np.kron(np.eye(n),A)+V/E_target
# Q=Hamiltonian_TISE/E_target+target*np.eye(n**2)
#
# def prec(x,*args):
...
print("Condition number before preconditioning:",np.linalg.cond(Q))
