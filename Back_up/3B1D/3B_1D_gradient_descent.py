...
def prec_func_T(x,*args):
    #function approximates w in (A-sigma*I)w=x
    C=np.reshape(x,(N_x,N_y))
    C=np.asarray(C, dtype=np.float64)
    X = solve_sylvester(B.T,A.T, C.T,schur_A,schur_B)
    result_test=np.reshape(X.T,(N_x*N_y,1))
    return result_test
...
Q=np.kron(B,np.eye(N_y))+np.kron(np.eye(N_x),A)+V
# Q=Hamiltonian_TISE/E_target-target*sp.sparse.eye(N_x*N)
def geometric_prec(x, *args):
    x=np.reshape(x,(N_x*N_y,1))
    print("x shape:", x.shape)
    e_til=prec(x)
    e_til=np.reshape(e_til,(N_x*N_y,1))
    x_k=np.zeros(N_x*N_y)
    x_k=np.reshape(x_k,(N_x*N_y,1))
    iterations=5
    print("shape before:", x_k.shape)
    ...
    for i in range(iterations):
        ...
        print("shape after:", x_k.shape,i)
        x_k=x_k+tau*Q.T@prec_T(residual)
        x_k=np.reshape(x_k,(N_x*N_y,1))

    #hij gebruikt de x_k vector niet helaas weet niet hoe moet fixen

    print("final residual:",np.linalg.norm(residual))

    print("shapies", x_k.shape)
    x=prec(x)
    x_k=np.reshape(x_k,(N_x*N_y,1))
    # x_k=np.ones((N_x*N_y,1))
