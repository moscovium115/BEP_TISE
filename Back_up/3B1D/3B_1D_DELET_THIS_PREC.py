...
inv_V_shifted=1/((V_E).diagonal()-target)
inv_V_shifted=sp.sparse.diags(inv_V_shifted)
def prec_A(x,*args):
    x_1=inv_V_shifted@x
    x_1=np.reshape(x_1,(N_x*N_y,1))
    ans_vec=x_1-E_target*inv_V_shifted@(Hamiltonian_TISE@x_1-V@x_1)
    ans_vec=x_1
    ans_vec=np.reshape(ans_vec,(N_x*N_y,1))
    # print(x_1.shape)
    return ans_vec
...
for test in range(num_tests):
    ...
    eigenval,eigenvec=jdqr.jdqr(Hamiltonian_TISE/E_target,num=1,target=-2.71,tol=1e-10,return_eigenvectors=True,subspace_dimensions=(20,50),maxit=2500,prec=prec_A)
